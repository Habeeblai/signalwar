...
const history = [
  { name: 'BNB Masters XV', ended: '2 days ago', size: 32, rounds: 5, winner: '🤖 IronFist', prize: '10,000 tBNB' },
  { name: 'Battle Royale XV', ended: '5 days ago', size: 16, rounds: 4, winner: '👻 GhostSignal', prize: '5,000 tBNB' },
  { name: 'Alpha Championship I', ended: '1 week ago', size: 64, rounds: 6, winner: '🧠 QuantumFade', prize: '25,000 tBNB' },
];
    ...
    <div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 2rem 1.5rem', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem' }}>
        {[
          ...
          { num: '142K tBNB', label: 'Total Prize Pool', sub: '↑ 24K added today' },
              ...
              <div style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 0, alignItems: 'flex-start', minWidth: 600 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', paddingLeft: 16, marginTop: 100 }}>
                        <div style={{ textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: 10, color: 'var(--amber)', fontWeight: 700, marginTop: 4 }}>7,000 tBNB</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {(expTabs[t.id] === 'prizes' || expTabs[t.id] === undefined) && (expTabs[t.id] === 'prizes') && (
                  <div style={{ padding: '1.25rem', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem' }}>
                    {[{ icon: '🥇', label: '1st Place', pct: '70%', mnt: t.id === 't1' ? '7,000 tBNB' : '3,500 tBNB' }, { icon: '🥈', label: '2nd Place', pct: '20%', mnt: t.id === 't1' ? '2,000 tBNB' : '1,000 tBNB' }, { icon: '🥉', label: '3rd Place', pct: '10%', mnt: t.id === 't1' ? '1,000 tBNB' : '500 tBNB' }].map(p => (
                ...
                {expTabs[t.id] === 'bets' && (
                  <div style={{ padding: '1.25rem' }}>
                    ...
                    <div style={{ marginTop: '1rem', display: 'flex', gap: 8 }}>
                      <input style={{ flex: 1, background: 'var(--bg-card2)', border: '1px solid var(--border)', color: 'var(--text)', padding: '8px 12px', borderRadius: 8, fontSize: 13, outline: 'none' }} placeholder="Wager (tBNB)" type="number"/>
      ...
      {modalOpen && (
        <div onClick={e => { if (e.target === e.currentTarget) setModalOpen(false); }} style={{ position: 'fixed', inset: 0, background: 'hsla(224,35%,2%,.85)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '1.75rem', width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <input className="input-field" placeholder="e.g. BNB Grand Prix" style={{ marginBottom: '1rem' }}/>
            ...
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: '.5rem' }}>Prize Pool (tBNB)</div>
            <input className="input-field" type="number" placeholder="Min 1,000 tBNB" style={{ marginBottom: '.75rem' }}/>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: '.5rem' }}>Entry Fee (tBNB)</div>
