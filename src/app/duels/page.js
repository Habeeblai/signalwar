...
const duels = [
  { id: '0047', status: 'live', agentA: { ava: '🤖', name: 'IronFist', strategy: 'Momentum', pnl: '+12.4%', pos: true }, agentB: { ava: '🦾', name: 'NightOwl', strategy: 'RSI Reversal', pnl: '+8.7%', pos: true }, asset: 'ETH / BTC', time: '18:25', pot: '500 MNT', bets: 12 },
  { id: '0048', status: 'live', agentA: { ava: '🌙', name: 'MoonChaser', strategy: 'MACD Cross', pnl: '+6.2%', pos: true }, agentB: { ava: '🚨', name: 'RedAlert', strategy: 'Mean Reversion', pnl: '−2.1%', pos: false }, asset: 'BTC / SOL', time: '02:47', pot: '1,200 MNT', bets: 34 },
  { id: '0049', status: 'live', agentA: { ava: '👻', name: 'GhostSignal', strategy: 'Vol. Breakout', pnl: '+18.9%', pos: true }, agentB: { ava: '🧘', name: 'ZenMode', strategy: 'Swing Trader', pnl: '+14.3%', pos: true }, asset: 'SOL / ARB', time: '58:12', pot: '350 MNT', bets: 7 },
  { id: '0050', status: 'live', agentA: { ava: '⚡', name: 'DeltaSniper', strategy: 'Scalper', pnl: '−4.1%', pos: false }, agentB: { ava: '🧠', name: 'QuantumFade', strategy: 'Contrarian', pnl: '+9.2%', pos: true }, asset: 'PEPE / MNT', time: '11:33', pot: '800 MNT', bets: 19 },
];

const openDuels = [
  { id: '0051', agentA: { ava: '⚡', name: 'DeltaSniper', strategy: 'Scalper · 20x' }, asset: 'PEPE / MNT', duration: '1 Hour', pot: '800 MNT', expires: '2h', addr: '0x4f…2a4b' },
  { id: '0052', agentA: { ava: '🧠', name: 'QuantumFade', strategy: 'Contrarian · 5x' }, asset: 'BTC / ETH', duration: '15 Min', pot: '2,000 MNT', expires: '45m', addr: '0x9c…1b2a' },
];
...
const history = [
  ...
  { id: '0036', agents: 'ZenMode vs MoonChaser', asset: 'BTC/MNT', duration: '6 Hours', result: 'Win', pnl: '+$1,240' },
...
export default function DuelsPage() {
  ...
  const payout = wager && parseFloat(wager) > 0
    ? (parseFloat(wager) * parseFloat(betTypes[selectedBet].odds)).toFixed(1) + ' MNT'
    : '—';
      ...
      {modalOpen && (
        <div onClick={e => { if (e.target === e.currentTarget) setModalOpen(false); }} style={{ position: 'fixed', inset: 0, background: 'hsla(224,35%,2%,.85)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '1.75rem', width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: '.5rem' }}>Wager Amount (MNT)</div>
            <input className="input-field" type="number" placeholder="Enter wager amount (min 10 MNT)" style={{ marginBottom: '.75rem' }}/>
            ...
            <select className="input-field" style={{ marginBottom: '.75rem', cursor: 'pointer' }}>
              <option>BTC / ETH</option><option>ETH / SOL</option><option>SOL / ARB</option><option>MNT / ETH</option><option>PEPE / DOGE</option>
            </select>
