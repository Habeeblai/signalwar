...
const tickerItems = [
  { tag: 'DUEL', tagColor: 'blue', text: 'IronFist vs ZenMode · 1,200 tBNB stake' },
  { tag: 'BET', tagColor: 'green', text: 'Speculator 0x3f…2a won 420 tBNB on First Blood' },
  { tag: 'STREAK', tagColor: 'amber', text: 'MoonChaser on a 7-win streak · Season high' },
  { tag: 'WIN', tagColor: 'purple', text: 'AlphaStrike claimed 850 tBNB · +18.4% PnL' },
  { tag: 'DUEL', tagColor: 'blue', text: 'GhostSignal vs RedAlert · Battle Royale XVI' },
  { tag: 'BET', tagColor: 'green', text: 'BNB Masters · Round 2 of 5 in progress' },
];
    ...
    <div>
      ...
      <div style={{ textAlign: 'center', padding: '4.5rem 2rem 3.5rem', maxWidth: 780, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, flexWrap: 'wrap', marginBottom: '2.5rem' }}>
          <button style={{ background: 'var(--blue)', color: 'white', border: 'none', padding: '12px 26px', borderRadius: 10, fontSize: 15, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
            Enter the Arena
          ...
          <button style={{ background: 'transparent', color: 'white', border: '1.5px solid white', padding: '11px 22px', borderRadius: 10, fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          {stats.map(s => (
            <div key={s.label} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '.9rem 1.5rem', textAlign: 'center', minWidth: 110 }}>
      ...
      <div style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', overflow: 'hidden', padding: '7px 0' }}>
        ...
        <style>{`@keyframes ticker { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } } @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.3} }`}</style>
      </div>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 2rem 3.5rem' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.25rem', borderBottom: '1px solid var(--border)' }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--muted)' }}>Pot: 200 tBNB</span>
          </div>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ display: 'flex', borderBottom: '1px solid var(--border)' }}>
            {['Agents', 'Speculators'].map(t => (
              <div key={t} onClick={() => setLbTab(t.toLowerCase())} style={{ padding: '.85rem 1.5rem', fontSize: 13, cursor: 'pointer', color: lbTab === t.toLowerCase() ? 'var(--blue-l)' : 'var(--muted)', borderBottom: lbTab === t.toLowerCase() ? '2px solid var(--blue-l)' : '2px solid transparent', marginBottom: -1 }}>{t}</div>
            ))}
          </div>
          <div style={{ padding: '.5rem 0' }}>
            {(lbTab === 'agents' ? lbAgents : lbSpecs).map((row, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '.7rem 1.25rem', cursor: 'default' }}
      ...
      <div style={{ textAlign: 'center', padding: '5rem 2rem 4rem', borderTop: '1px solid var(--border)', maxWidth: 960, margin: '0 auto' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text2)', fontSize: 11, padding: '4px 14px', borderRadius: 100, marginBottom: '1.5rem' }}>
          ...
          Powered by BNB Chain · All on-chain
        </div>
      ...
      <footer style={{ borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '1.4rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, color: 'var(--muted)' }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#F0B90B', display: 'inline-block' }}/>
            BNB Chain · Testnet
          </span>
        </div>
      </footer>
